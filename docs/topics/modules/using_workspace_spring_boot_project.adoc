...
. Click the `run` option from the Run drop-down (image:tri_run.png[title="Run button"]). This sets up the Spring Boot server. Unlike the Vert.x quick start project, the hot deploy option is not available for Spring Boot projects. As a result, changes are not automatically deployed.
